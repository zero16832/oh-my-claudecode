...
describe('BitbucketProvider', () => {
    ...
    describe('viewPR', () => {
        it('fetches PR via curl and parses response', () => {
            ...
            expect(mockExecFileSync).toHaveBeenCalledWith('curl', ['-sS', '-H', 'Authorization: Bearer test-token', 'https://api.bitbucket.org/2.0/repositories/user/repo/pullrequests/5'], expect.objectContaining({ encoding: 'utf-8', timeout: 10000 }));
        ...
        it('uses Basic auth when username and app password are set', () => {
            ...
            expect(mockExecFileSync).toHaveBeenCalledWith('curl', ['-sS', '-H', `Authorization: ${expectedAuth}`, expect.stringContaining('pullrequests/1')], expect.any(Object));
        });
    ...
    describe('viewIssue', () => {
        it('fetches issue via curl and parses response', () => {
            ...
            expect(mockExecFileSync).toHaveBeenCalledWith('curl', ['-sS', '-H', 'Authorization: Bearer test-token', 'https://api.bitbucket.org/2.0/repositories/user/repo/issues/3'], expect.objectContaining({ encoding: 'utf-8' }));
            expect(result).toEqual({
                title: 'Bug report',
