...
import { initJobDb, closeJobDb, closeAllJobDbs, isJobDbInitialized, upsertJob, getJob, } from '../job-state-db.js';
...
describe('job-state-db deprecation warnings', () => {
    ...
    describe('getDb deprecation warning', () => {
        it('should emit deprecation warning when getDb() called without cwd and multiple DBs are open', async () => {
            ...
            expect(warnings.some((w) => typeof w === 'string' && w.includes('DEPRECATED') && w.includes('getDb()'))).toBe(true);
            warnSpy.mockRestore();
        });
        it('should not emit deprecation warning with single DB open', async () => {
            ...
            const warnings = warnSpy.mock.calls.map(c => c[0]);
            expect(warnings.some((w) => typeof w === 'string' && w.includes('DEPRECATED') && w.includes('getDb()'))).toBe(false);
            warnSpy.mockRestore();
        });
    });
    ...
    describe('closeJobDb without cwd deprecation', () => {
        it('should emit deprecation warning when called without cwd', async () => {
            ...
            closeJobDb(); // no cwd
            const warnings = warnSpy.mock.calls.map(c => c[0]);
            expect(warnings.some((w) => typeof w === 'string' && w.includes('DEPRECATED') && w.includes('closeJobDb()'))).toBe(true);
            warnSpy.mockRestore();
        });
    });
});
//# sourceMappingURL=job-state-db-deprecation.test.js.map