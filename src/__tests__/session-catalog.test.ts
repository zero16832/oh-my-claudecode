...
vi.mock('../analytics/cost-estimator.js', () => ({
  calculateCost: vi.fn((input: { inputTokens: number; outputTokens: number }) => ({
...
const mockLine = (sessionId: string, timestamp: string, model: string, input: number, output: number, cacheCreation = 0, cacheRead = 0, agentName?: string) =>
  JSON.stringify({
    sessionId,
    timestamp,
    modelName: model,
