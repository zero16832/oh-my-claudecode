...
export function isValidTokenUsage(record: unknown): record is TokenUsage {
...
export class SessionCatalog {
   ...
   */
  async getSessions(limit?: number): Promise<CatalogSession[]> {
   ...
   */
  async getSession(sessionId: string): Promise<CatalogSession | null> {
    const records = await this.readTokenLog();
  ...
  private buildCatalogSession(sessionId: string, usages: TokenUsage[]): CatalogSession {
    let startTime = usages[0].timestamp;
    let endTime = usages[0].timestamp;
    let totalTokens = 0;
    let totalCost = 0;
    const agentBreakdown: Record<string, { tokens: number; cost: number }> = {};
    const modelBreakdown: Record<string, { tokens: number; cost: number }> = {};

    for (const usage of usages) {
      // Track time range
      if (usage.timestamp < startTime) startTime = usage.timestamp;
      if (usage.timestamp > endTime) endTime = usage.timestamp;
  ...
  private async readTokenLog(): Promise<TokenUsage[]> {
    try {
      ...
      if (error instanceof Error && 'code' in error && (error as NodeJS.ErrnoException).code !== 'ENOENT') {
        console.warn('[session-catalog] Failed to read token log:', (error as Error).message);
      }
      return [];
    }
  }
...
// Singleton
let catalogInstance: SessionCatalog | null = null;

export function getSessionCatalog(): SessionCatalog {
  if (!catalogInstance) {
